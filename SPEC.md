...
| D48 | 비번 self-service reset | Brevo SMTP 동작 확인 후 사용자가 admin 호출 안 하고 직접 비번 재설정 가능. /login 에 "비밀번호를 잊으셨나요?" 링크 → /forgot-password 에 이메일 입력 → `supabase.auth.resetPasswordForEmail(email, { redirectTo: '<origin>/auth/callback?redirect=/reset-password' })` → Supabase 가 reset 메일 발송 → 사용자 클릭 → /auth/callback 에서 세션 만들고 /reset-password 로 redirect → 새 비번 입력 → `updateUser({ password })` + `users.password_set=true` → /map. /forgot-password 는 PUBLIC_PATHS, /reset-password 는 인증 필요 (callback 통해 진입). admin 의 임시비번 reset (D38) 도 그대로 유지 — 이메일 자체가 막힐 때 폴백 |
