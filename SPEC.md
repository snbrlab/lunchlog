...
| D57 | 픽셀 기반 동적 클러스터링 | D56 의 "정확히 같은 좌표" 만 묶던 제약을 줌 레벨 따라 동적으로 확장. **알고리즘**: kakao `map.getProjection().containerPointFromCoords()` 로 각 식당의 화면 픽셀 위치 계산. 그리디 (O(n²), 100개대까지 충분) — 첫 식당부터 cluster seed, 후보를 모든 기존 cluster centroid (누적 평균) 와 비교해서 40px 이내면 흡수, 아니면 새 cluster. 핀은 cluster centroid 위치에 표시. **재계산 트리거**: 카카오 map `idle` 이벤트 (팬/줌 종료 시) 에서 `mapVersion++` → 핀 effect 의 deps 에 들어가서 자동 재실행. **Popover 키 안정**: 그룹 key 를 `solo_{id}` 또는 `cluster_{정렬된 멤버 id join}` 으로 → 같은 멤버 묶음이면 줌/팬 해도 동일 key 라 popover 유지. 멤버가 바뀌면 (split/merge) 자동 닫힘. Popover 헤더 문구 "이 근처 N곳" |
