...
| D49 | 사무실 / 건물 admin 추가 | /admin/buildings 페이지에 새 사무실 / 건물 추가 폼. 그동안 마이그레이션 (`add_office_*.sql`) 으로만 추가 가능했던 걸 admin 이 UI 에서 직접 추가 가능. 이름 + lat/lng 입력 → `createOffice` / `createBuilding` server action. slug 는 자동 생성. display_order 는 해당 office 의 max+1 자동. 좌표는 대략값 박고 자동 보정 (KAKAO_REST_KEY) 으로 정정 가능. invalidateOfficesCache 호출로 캐시 즉시 무효화 |
