...
| D45 | 카카오맵 url fallback 등록 | 카카오 Local API (keyword search) 에 indexing 안 된 식당 (UGC, 비즈니스 미등록 등) 도 등록 가능하게. KakaoPlacesSearch 하단의 details 섹션에서 `place.map.kakao.com/{id}` url 또는 place 번호 직접 입력 → server action `fetchKakaoPlaceFromUrl` 이 비공식 endpoint `https://place.map.kakao.com/main/v/{id}` 호출해 name/coord/address 추출 → 기존 검색 결과와 동일한 KakaoPlaceItem 형태로 onSelect 호출. ⚠️ 비공식 API 의존 (P18 영업시간과 동일 endpoint) — 깨질 시 fallback 의 fallback 으로 admin 직접 입력 검토 |
