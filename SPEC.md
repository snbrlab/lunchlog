...
| D65 | 첫 화면 속도 3종 | **(a) `/` 서버 리다이렉트**: 기존 client-side `useEffect` → `redirect('/map')` 서버 컴포넌트. 첫 방문 시 빈 페이지 + JS hydrate + 재이동 round-trip 통째로 제거. proxy.ts 가 이미 인증 가드라 안전. **(b) 공지 배너 쿼리 캐싱**: D59 가 `(app)/layout` 에 넣은 active 공지 fetch 가 전 페이지 SSR 마다 DB 왕복 → `lib/cache/announcements.ts` `getCachedActiveAnnouncements` (`unstable_cache` tag `announcements`, 1h). 작성/내리기/삭제 action 에서 `invalidateAnnouncementsCache()`. layout 의 await 가 헤더/껍데기 렌더 막던 것 해소. **(c) Kakao preconnect**: root layout `<head>` 에 `preconnect` + `dns-prefetch` (`https://dapi.kakao.com`) — SDK 연결을 HTML 파싱과 병렬화, /map 첫 지도 표시 100~300ms 단축 |
| D66 | 전체메일 다이제스트 (재참여) | admin 이 /admin/broadcast 에서 현황 통계 + 수신자별 "나의 마지막 commit" 을 매거진풍 HTML 로 발송. **통계**: users / restaurants / reviews count (service-role, head:true). **개인화**: 수신자별 마지막 commit (reverted 제외) → 한 통 broadcast 아니라 사용자별 렌더+개별 발송. **발송**: Brevo transactional API (`api.brevo.com/v3/smtp/email`, Supabase Auth SMTP 와 별개). env `BREVO_API_KEY` / `BREVO_SENDER_EMAIL` (선택, 없으면 발송 비활성 + 경고). 순차 + 120ms 딜레이, 실패해도 계속 + 실패 목록 리턴. **안전장치**: "나에게 테스트" 먼저 → 전체 발송은 `전체 {N}` 타이핑 확인. Outlook Safe Links (D30) 회피로 링크는 CTA 1개만. **카피/디자인**: 제목 "🍱 런치로그 - 오늘 점심 드셨어요?", 슬로건 "오늘 먹은 한 끼를 커밋합니다". git 뉴스레터 마스트헤드 + 통계 3-up 카드를 메인 후크 ("이만큼 쌓였으니 구경 와", CTA→/log) + 개인 1일1커밋 잔디(마지막 커밋 N일 전 계산) 보조. **미리보기**: `getBroadcastPreviewHtml(pickedIds)` — 발송 없이 본인 데이터로 렌더한 HTML 을 admin 페이지 iframe(srcDoc) 에 표시. **흥미로운 commit 첨부**: `listRecentCommits()` (최근 60, reverted 제외) 픽커에서 admin 이 최대 5개 선택(순서 유지) → `PickedCommit[]` 이 "👀 이런 한 줄도 있어요" 섹션으로 메일에 삽입. preview/test/full 발송 모두 picks 반영 |
| D67 | 사이트 URL 환경변수화 + 도메인/배포 이전 가이드 | 하드코딩이던 사이트 URL (`broadcast.ts` 의 `SITE_URL`) 을 `publicEnv.siteUrl` 로 일원화 — `NEXT_PUBLIC_SITE_URL` 환경변수, 미설정 시 `https://lunchlog-rho.vercel.app` fallback, 끝 슬래시 제거. 도메인 구매/배포처 이전 시 수정 지점은 `docs/DOMAIN_AND_DEPLOY.md` 에 정리: ① `NEXT_PUBLIC_SITE_URL` env, ② Supabase Auth URL Configuration (Site URL + Redirect allow list, 안 하면 OTP/리셋 깨짐), ③ Kakao Developers Web 사이트 도메인 (안 하면 지도 안 뜸). 안 건드려도 되는 것: Supabase/Kakao REST/Brevo 키 |
| D68 | 사용자 임시 근무지 좌표 직접 지정 | 사용자 제보 — 프로젝트 투입으로 공유 오피스에서 근무 중인데 등록 건물 목록에 없어 거리 계산이 안 맞음. **DB 컬럼**: `users.custom_lat / custom_lng double precision` (둘 다 nullable, 체크 제약 "둘 다 set 이거나 둘 다 null"). **마이페이지 프로필 편집** 에 "건물 말고 다른 위치에서 근무 중이에요" 토글 → 카카오 장소 검색(`KakaoPlacesSearch` 재사용, 서울 시청 좌표 bias) 으로 좌표/이름 선택. **origin 우선순위**: `custom_lat/lng > building > LG사이언스파크 fallback` — /map page.tsx 에서 적용. 한 번 설정하면 어느 기기에서든 일관 (브라우저별 localStorage 안 씀). `updateProfile` action 이 한쪽만 채워진 케이스 거부 + 좌표 범위 검증 |
| D69 | 제보 댓글 스레드 (ping-pong) | 기존 `reports.admin_note` 단일 필드 덮어쓰기 → 확인중/완료 단계별 코멘트 보존 안 됨 + 사용자 답글 불가능 문제. **`report_comments` 테이블** (report_id cascade, author_id set null, body 1~2000자) + RLS (제보자 본인 또는 admin read/write). **ping-pong 룰**: 직전 댓글 author 가 본인이면 거부. 댓글 0개면 admin 만 첫 답글 가능 (제보 본문이 곧 사용자 게시본). server action `addReportComment(reportId, body)` 에서 enforce. `deleteReport(reportId)` (admin 전용) — cascade 로 댓글 함께 삭제. **공유 컴포넌트** `components/reports/ReportThread.tsx` — chat 스타일 좌/우 버블 (내 거 우측 검정, admin 좌측 amber), "내 차례 / 응답 대기" 힌트 + 비활성 input. `/report` (사용자) 와 `/admin/reports` 둘 다 사용. /admin/reports 는 status 변경 버튼 + 🗑 삭제 버튼 유지. **노티**: `report_comments` insert 트리거 → admin → 제보자 / 사용자 → 모든 admin 에게 `'report_comment'` 타입 노티 (NotificationToast UI 새 카드, from='admin' 이면 /report, from='user' 면 /admin/reports 로 링크). 기존 `notify_on_report_admin_update` 트리거는 상태 변경 노티 용도로 유지. **`resolved` 상태 잠금**: addReportComment 가 status='resolved' 면 거부, UI input 비활성 + 안내 (admin 한텐 "상태를 확인중으로 되돌리세요") |
| D70 | 작성자 뱃지 시스템 (sticky) | D36 ranking 공개 전 캐릭터 도입. **30개 뱃지** 6축: 활동량(commit 누적 5단), 꾸준함(streak 4단), 개척(혼자 commit 식당 3단), 다양성(cuisine 그룹 3단), 시간대(점심반장/야근요정), cuisine 특화(13그룹 × 1단). 메타는 `lib/badges.ts` TS 상수 (label/emoji/axis/tier/threshold/description, 캐릭터형 농담 톤). **DB**: `user_badges (user_id, code, awarded_at)` + unique(user, code), RLS 누구나 read, INSERT 정책 없음 (security definer 함수만). sticky — 한 번 받으면 영구. `users.primary_badge_code` (사용자 선택 대표 뱃지, /log 에 노출, D50 column GRANT 추가). **award_badges_for(uid)** PL/pgSQL — count/streak(KST 일자 연속)/pioneer/distinct cuisine groups/meal_time 비중/그룹별 commit 계산 → 미보유 분만 insert + `notifications` 'badge_earned' insert. 리뷰 insert/UPDATE(revert flip) 트리거로 자동 호출. 마이그레이션 끝에 backfill (모든 사용자 1회 호출). **UI**: `BadgeCollection` (Steam 도감, /me — axis 별 section, 받은 거 amber, 잠긴 거 회색 🔒 ???), `BadgeGrid` (/u/[id], 받은 거만), `BadgeChip` (/log row 작성자 옆 대표 뱃지 1개). server action `setPrimaryBadge(code\|null)` — 소유 검증 후 users.primary_badge_code 갱신. NotificationToast 'badge_earned' 카드 (이모지 큼직 + 라벨 + 캐릭터형 description, 클릭 시 /me 로) |
| D71 | 지역별 대장 (롤링 왕관) | sticky 뱃지(D70)와 별개 — office 당 1명만 가지는 빼앗기는 왕관. **기준**: 활성 commit 수 (reverted=false) for 식당.office_id = 해당 office. admin 후보 제외. 동률 시 먼저 그 카운트에 도달한 사람 유지 (max created_at 작은 사람). **식당 location 기준** (사용자 소속 무관) — 마곡 사람이 오산 식당에 많이 쓰면 오산 대장. "그 지역 식당을 가장 잘 아는 사람" 의미. **DB**: `region_champions (office_id PK, user_id, commit_count, since_at)` + RLS read all, INSERT/UPDATE 정책 없음 (security definer 만). `recompute_region_champion(office_uuid)` 함수 — 새 1위 계산 후 변동 시 row 갱신 + 새 대장에게만 `notifications` 'region_champion' insert (빼앗긴 사람은 조용히). 리뷰 insert/UPDATE(revert)/DELETE 트리거에서 해당 식당의 office 재계산. 마이그레이션 끝 backfill. **UI**: `RegionCrowns` 컴포넌트 (👑 rose chip "○○ 대장") — **/me, /u/[id] 헤더에만 노출**. /log row 에는 perf 부담 (PostgREST reverse embed 가 두 LEFT JOIN 추가) 때문에 표시 안 함 — primary_badge_code chip 만. NotificationToast 'region_champion' 카드 (rose 톤, "🎉 ○○ 대장이 됐어요!" + commit 수). **최소 commit 10 컷**: 1~2 commit 으로 왕관 받는 거 방지 — `having count(*) >= 10`. 부족하면 왕좌 비움. **인덱스 보강** (migration 20260520000003): `idx_reviews_active_author_created (author_id, created_at desc) WHERE reverted=false` 부분 인덱스 — D70/D71 활성 리뷰 집계 가속. **/admin/champions 페이지** — 사무실별 현재 대장 표 (admin 만, /u/[id] 링크). **/log primary_badge 캐시 동기화**: `setPrimaryBadge` / `updateProfile` 가 `invalidateReviewsLogCache()` 호출 — 마이페이지에서 대표 뱃지/닉네임/이모지 바꾸면 /log 에 즉시 반영 |
| D72 | restaurants.office_id 의미를 지리적으로 | D71 의 "지역" 정의 부정확 — 기존 `restaurants.office_id` 는 D43 에서 "처음 등록한 사람의 office_id" 였음. 평택 사람이 마곡 식당 첫 등록하면 그 마곡 식당이 office=평택 으로 잡혀 D71 region_champions 계산이 흔들림. **변경**: `restaurants.office_id` 의미를 "식당 좌표 기준 가장 가까운 office" 로 재정의. **마이그레이션** 20260520000005: SQL 헬퍼 `nearest_office_id(lat, lng)` (offices.default_lat/lng 거리 비교, 평면 근사로 빠르게) + 모든 기존 restaurants 의 office_id 를 좌표 기반으로 backfill UPDATE + region_champions 전 office 재계산. **신규 등록 로직**: `restaurants/new/actions.ts` 의 `office_id: profile.office_id` 를 `getCachedOffices` 에서 가장 가까운 office 찾아 매핑하도록 변경 (사용자 소속과 무관). 결과적으로 D43 의 "office_id = 등록자 office" 메타데이터 의미는 폐기됨 |
