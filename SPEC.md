...
| D45 | 직접 입력 fallback 등록 | 카카오 Local API (keyword search) 에 indexing 안 된 식당 등록용. 처음엔 비공식 endpoint `place.map.kakao.com/main/v/{id}` 시도했으나 카카오가 막아둠 (404). 공식 Kakao Local REST geocoding (`/v2/local/search/address.json`) 으로 전환 — 사용자가 이름/도로명 주소 직접 입력 → 주소를 좌표로 변환 (KAKAO_REST_KEY 사용). 카카오맵 url 은 선택 입력 (디테일 패널 외부 링크용). KakaoPlacesSearch 하단 details 섹션에서 노출 |
