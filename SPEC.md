...
| D68 | 임시 근무지 (공유 오피스) origin 덮어쓰기 | 제보: 공유 오피스 등 등록 안 된 건물에서 일할 때 거리/회사 마커 기준점이 등록된 건물이라 부정확. **저장**: `MapShell` `customOrigin` state + `localStorage.lunchlog.custom_origin.v1` 보존 (DB 안 감 — 임시 용도, 브라우저별). 우선순위 `customOrigin ?? origin(building)`. **두 가지 입력 경로**: ① 기존 📍 GPS 버튼이 시점 이동 + GPS 좌표를 origin 으로 저장하도록 확장, ② 🖊️ 토글 버튼으로 "지도 클릭으로 지정" 모드 — 다음 지도 클릭 좌표를 origin 으로 (안내 배너 + crosshair 커서, 클릭 1회 후 모드 자동 해제). **active 표시**: 좌상단 amber 배지 "📍 사용자 지정 위치 · ✕ 초기화" (모바일 햄버거와 안 겹치게 `max-lg:left-14`). 초기화는 localStorage 비우고 등록된 건물로 복귀 |
