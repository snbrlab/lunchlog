-- SECURITY FIX: merge_restaurants 가 authenticated 에게 grant 되어 있고 security definer 라
-- 아무 로그인 유저나 브라우저에서 supabase.rpc('merge_restaurants', ...) 로 임의 식당을
-- 병합/영구삭제할 수 있었음 (admin 게이트가 서버액션에만 있고 RPC 자체엔 없었음).
--
-- 함수 첫 줄에 is_admin() 가드 추가. admin 서버액션은 호출자 JWT 로 RPC 를 부르므로
-- (createSupabaseServerClient) 그대로 통과하고, 비관리자 직접 호출만 차단됨.
-- 나머지 본문은 migration 13(union metadata) 와 동일.
